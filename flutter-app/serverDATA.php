...
class serverDATA
{
    ...
    public function new_product($data)
    {
        ...
        $new_product = $this->connection->prepare("SELECT * FROM product ORDER BY order_number DESC limit 10 ");
        $new_product->execute();
    ...
    public function order_product($data)
    {
        ...
        $order_product = $this->connection->prepare("SELECT * FROM product ORDER BY id DESC limit 10 ");
    ...
    {
        $product_id = array_key_exists('product_id', $data) ?
            $data['product_id'] : 0;
    ...
    public  function  get_comment($data){
        ...
        $comment=$this->connection->prepare("SELECT * FROM comment where product_id=? order by id DESC limit 10");
